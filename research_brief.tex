...
A student's grades depend on effort and ability. They may also depend on the students
they choose to connect with online. A social network gives access to information, advice, study partners, and
motivation. So where a student sits in the network may matter for grades. We ask
whether it does.

\section*{Research question}

In a directed student ``like'' network, does a student's outgoing network reach predict
their next-period grades, beyond past GPA and direct contacts?
...
We combine the two. The high-school cohort supplies six genuine GPA measurements per
student, so it can test whether centrality at time $t$ predicts GPA at $t+1$ while
holding past GPA and direct-contact composition fixed. The university cohorts have a
single static GPA measurement and are used only for descriptive comparisons.
...
social part: $g_{ij}=1$ if student $i$ liked student $j$, and $\phi>0$ means that
the effort of a student whom $i$ chose to connect with raises $i$'s return to effort.
...
\begin{itemize}
  \item Small $\phi$: only direct contacts count. Centrality is almost the same as
        outgoing degree.
...
For the high-school network, we compute centrality at each $c$, then run
\[
\begin{aligned}
\mathrm{GPA}_{i,\,t+1} = {}& \beta_0 + \beta_1\,\mathrm{Katz}_{i,t}
  + \beta_2\,\mathrm{GPA}_{i,t}
  + \beta_3\,\overline{\mathrm{GPA}}^{\,\text{contacts}}_{i,t} \\[2pt]
  & {} + \beta_4\,\mathrm{InDeg}_{i,t} + \beta_5\,\mathrm{OutDeg}_{i,t}
  + \gamma_t + \varepsilon_{i,t}.
\end{aligned}
\]
Each control has a job. Average contact GPA is Smirnov and Thurner's variable, so
$\beta_1$ is net of contact composition. In- and out-degree are direct-link controls,
so $\beta_1$ is net of them too. The real question is whether indirect reach adds
anything beyond direct contacts. We run this at every $c$, using $c = 0.85$ as the
primary specification, and check incoming status and reciprocal ties as alternatives.
...
\begin{enumerate}
  ...
  \item \textbf{Past GPA is controlled.} We compare students who currently have the same
        grade.
  \item \textbf{Panel data in high school.} The main test uses the high-school
        panel. The university cohorts cannot provide this timing check because their
        GPA is static.
\end{enumerate}
...
\begin{itemize}
  \item $\beta_1 > 0$: students with greater outgoing reach gain from more access to information and help.
